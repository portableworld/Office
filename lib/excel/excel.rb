# Should catch this if it isn't on the system
require 'win32ole'
require_relative '../Excel/Module'

module Office
  class Excel
    attr_accessor :xl, :wb, :ws, :file_path
    ...
    def sheet(sheet_name)
      ...
      if [SHEET_HIDDEN, SHEET_VERY_HIDDEN].include? @ws.visible
        ...
        # TODO - Take a second parameter for unhiding it if it is hidden
      else
        @ws.select
      end
    ...
    def clear_sheet(start_cell = 'A2', end_column = nil)
   		if end_column.nil?
   			end_column = @ws.range('A1').end(TO_RIGHT).address
    ...
    def update_named_ranges
  		...
  		@wb.names.each do |r|
  			...
  			column = @wb.worksheets(sheet_name).range('A1').end(TO_RIGHT).address
